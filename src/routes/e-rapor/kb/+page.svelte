<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      const response = await fetch(`${PUBLIC_API_URL}/penilaian?tingkat=KB`, {
      ...
      if (response.ok && result.status) {
        ...
        dataSiswaKB = result.data.filter(item => item.status === 'Selesai');
      }
    } catch (error) {
...
<svelte:head>
  <title>E-Rapor KB - SIAKAD Al Hijrah</title>
</svelte:head>
  ...
  <div class="flex-1 flex flex-col overflow-hidden w-full relative">
    ...
    <header class="h-20 bg-white flex items-center justify-between px-6 md:px-10 shrink-0 border-b border-gray-100">
      <div class="flex items-center gap-4">
        ...
        <h1 class="text-xl font-bold text-gray-600">E-Rapor KB</h1>
      </div>
    ...
    <main class="flex-1 overflow-y-auto p-6 md:p-10 flex flex-col">
      <TableERapor bind:data={dataSiswaKB} isLoading={isLoading} tingkat="KB" />
    </main>
