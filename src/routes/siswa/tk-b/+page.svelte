<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      // Fetch Data Siswa
      const response = await fetch(`${PUBLIC_API_URL}/siswa?tingkat=TK B`, {
        headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
      });
      const result = await response.json();

      // Fetch Data Kelas
      const kelasRes = await fetch(`${PUBLIC_API_URL}/kelas?tingkat=TK B`, {
        headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
      });
      const kelasResult = await kelasRes.json();
      if (kelasRes.ok && kelasResult.status) {
        dataKelasTKB = kelasResult.data;
      }
      
      if (response.ok && result.status) {
        // 2. Mapping data dari database (snake_case) ke format komponen tabel (camelCase)
        dataSiswaTKB = result.data.map(item => ({
          id: item.id,
          nama: item.nama,
...
<svelte:head>
  <title>Siswa TK B - SIAKAD Al Hijrah</title>
</svelte:head>
  ...
  <div class="flex-1 flex flex-col overflow-hidden w-full relative">
    ...
    <header class="h-20 bg-white flex items-center justify-between px-6 md:px-10 shrink-0 border-b border-gray-100">
      <div class="flex items-center gap-4">
        ...
        <h1 class="text-xl font-bold text-gray-600">Siswa TK B</h1>
      </div>
    ...
    <main class="flex-1 overflow-y-auto p-0 flex flex-col">
      <TableSiswa bind:data={dataSiswaTKB} bind:dataKelas={dataKelasTKB} basePath="/siswa/tk-b" tingkat="TK B" isLoading={isLoading} />
    </main>
