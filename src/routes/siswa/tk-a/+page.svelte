<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      // Fetch Data Siswa
      const response = await fetch(`${PUBLIC_API_URL}/siswa?tingkat=TK A`, {
      ...
      if (response.ok && result.status) {
        ...
        dataSiswaTKA = result.data.map(item => ({
          id: item.id,
          nama: item.nama,
...
<svelte:head>
  <title>Siswa TK A - SIAKAD Al Hijrah</title>
</svelte:head>
  ...
  <div class="flex-1 flex flex-col overflow-hidden w-full relative">
    ...
    <header class="h-20 bg-white flex items-center justify-between px-6 md:px-10 shrink-0 border-b border-gray-100">
      <div class="flex items-center gap-4">
        ...
        <h1 class="text-xl font-bold text-gray-600">Siswa TK A</h1>
      </div>
    ...
    <main class="flex-1 overflow-y-auto p-0 flex flex-col">
      <TableSiswa bind:data={dataSiswaTKA} bind:dataKelas={dataKelasTKA} basePath="/siswa/tk-a" tingkat="TK A" isLoading={isLoading} />
    </main>
