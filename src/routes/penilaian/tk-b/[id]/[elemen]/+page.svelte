<script lang="ts">
  ...
  import { onMount } from 'svelte'; // Import onMount untuk memanggil API saat halaman dimuat
  ...
  async function hapusDataElemen() {
    ...
    try {
      ...
      const response = await fetch(`h${PUBLIC_API_URL}/penilaian/matriks/${penilaianId}/${elemenId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  <div class="flex-1 flex flex-col overflow-hidden w-full">
    <header class="h-20 bg-white flex items-center justify-between px-6 md:px-10 shrink-0 border-b border-gray-100">
      <div class="flex items-center gap-4">
        ...
        <a href="/penilaian/tk-b/{penilaianId}" class="flex items-center gap-2 text-gray-500 font-semibold">
